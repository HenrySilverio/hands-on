...
5. **Verifique divergências. Orçamento: três chamadas de ferramenta.**
   Com a ficha já gravada, compare o que a fonte exige com o que a base publica:
   leia o `INDEX.md`, abra no máximo **dois** arquivos que ele apontar como relacionados, e pare.

   Preencha o campo `divergencias` do frontmatter com o que de fato aconteceu:
   - `completa` — você leu os arquivos relacionados e comparou.
   - `parcial` — o orçamento acabou antes de cobrir tudo. Diga na seção o que ficou de fora.
   - `nao-verificada` — não foi possível. Diga por quê.

   Achou contradição? Registre os dois lados na seção de divergências e diga que precisa de
   decisão. **Não escolha um lado.** Não achou nada? Escreva "nenhuma divergência encontrada nos
   arquivos consultados" e liste quais foram.

   Falhou alguma leitura? Não insista: marque `parcial` ou `nao-verificada` e siga. A ficha já
   existe — esta etapa melhora o registro, não pode custar o registro.

6. **Mostre o conteúdo no chat**, diga o caminho e ofereça o commit e o pull request.

## Modo complementar: enriquecer uma ficha que já existe

Quando o usuário trouxer material novo sobre um assunto já registrado — um segundo print, a norma
publicada depois do voto, a resposta de uma área — **complemente a ficha existente** em vez de
criar outra.

1. Leia a ficha atual **uma vez**. Não existe? Diga isso e ofereça criar.
2. Some o que o material novo sustenta: obrigações, prazos, termos, impacto.
3. **Toda afirmação nova entra com o trecho que a sustenta**, igual à primeira rodada. Material
   complementar não afrouxa a regra de citação.
4. **Resolva lacuna só com evidência.** Se o novo material responde um `**[NÃO RESPONDIDO]**`,
   substitua pelo conteúdo e cite. Se não responde, a lacuna fica.
5. Atualize `obtido_em` e, se a confiança subiu de fato (voto virou resolução publicada), ajuste
   `confianca` — dizendo por quê em uma linha.

Não reescreva o que já estava certo, e não apague trecho literal anterior: fonte nova **acrescenta**
citação, não substitui a antiga.

Se o material novo **contradiz** o que está na ficha, não escolha: registre os dois lados na seção
de divergências e diga que precisa de decisão.
...
## Próximo passo, sempre com a frase pronta

Ao encerrar, diga o que vem depois e como pedir:

> **Próximo passo:** revise a ficha e aprove o pull request.
> Tem material novo sobre o mesmo assunto? Cole: `Complemente a ficha <slug> com isto.`
> A fonte gerou uma regra que o produto precisa seguir? Cole:
> `Registre a decisão que tomamos sobre <assunto>.`

O PO não deve precisar lembrar nome de agente nem caminho de pasta.
