...
## A fonte chega por três caminhos — identifique antes de agir

1. **Anexada nesta conversa** (PDF, imagem, documento): use o anexo. **Não procure arquivo em
   `inbox/`.**
2. **Colada como texto** no chat: use o texto.
3. **Arquivo em `inbox/`**: leia o arquivo.

Se o usuário citar um caminho e o anexo também existir, **o anexo vence** — e diga isso em uma
linha antes de seguir.

## Arquivo não encontrado: pare imediatamente

Se um caminho que você tentou ler não existe, **não tente de novo, nem com variação de nome, nem
depois de recarregar skill**. Responda:

> Não encontrei `<caminho>`. A fonte está anexada nesta conversa, colada como texto, ou eu devo
> criar o arquivo de entrada primeiro?

E encerre o turno. Ler duas vezes o mesmo caminho que falhou é sempre erro: o segundo resultado
será idêntico ao primeiro, e o que se ganha é uma sessão travada.

**Nunca repita a mesma chamada de ferramenta com os mesmos argumentos.** Se você percebe que está
prestes a fazer isso, o passo correto é parar e perguntar.

## Uma fonte por execução

Se o usuário fornecer mais de uma fonte na mesma mensagem — um PDF **e** um link, dois documentos,
um print **e** um texto — **pare e pergunte qual é a fonte canônica**. Não tente reconciliar duas
fontes numa ficha só: cada uma vira uma ficha, com sua própria URL, data e nível de confiança.

Material de apoio (um print que ilustra o que a norma diz) entra como observação na ficha da fonte
principal, nunca como segunda fonte a ser conferida.

## Falhou uma vez, pare

Rede corporativa bloqueia site externo com frequência, e conversão de PDF falha. Diante de
qualquer falha de ferramenta:

1. **Tente no máximo uma vez.** Não tente outra ferramenta, outro caminho, outra formulação.
2. Diga o que falhou, em uma linha.
3. Peça o que resolve: "não consegui abrir a URL — cole o texto das seções que interessam".

Insistir é o que transforma um bloqueio de trinta segundos em dez minutos sem entrega.

## Grave cedo, refine depois

Escreva a ficha **assim que tiver a citação literal e os metadados obrigatórios**, com
`**[NÃO RESPONDIDO]**` no que faltar. Só depois de o arquivo existir você procura divergências na
base e propõe melhorias.

Nunca deixe para gravar no fim: se algo travar no meio, o usuário fica sem nada, e o trabalho de
leitura que você já fez se perde junto com a sessão.

## Limite da busca na base

Ao conferir se o assunto já está coberto: leia o `INDEX.md`, abra no máximo três arquivos que ele
apontar, e pare. Se não achar, escreva "não encontrei cobertura na base" e siga. Varrer o
repositório inteiro não melhora a resposta e é o que faz a sessão se arrastar.
